...
*Setup instructions for a Gitian build of PocketChain Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the pocketchain
...
- [Building PocketChain Core](#building-pocketchain-core)
...
Clone the git repositories for pocketchain and Gitian.
...
in the pocketchain repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building PocketChain Core
----------------

To build PocketChain Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the pocketchain repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/pocketchain/.git/
...
For example:
```bash
URL=https://github.com/thrasher-/pocketchain.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit pocketchain=${COMMIT} --url pocketchain=${URL} ../pocketchain/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit pocketchain=${COMMIT} --url pocketchain=${URL} ../pocketchain/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit pocketchain=${COMMIT} --url pocketchain=${URL} ../pocketchain/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the pocketchain git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../pocketchain/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/pocketchain
SIGPATH=/some/root/path/pocketchain-detached-sigs

./bin/gbuild --url pocketchain=${BTCPATH},signature=${SIGPATH} ../pocketchain/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/pocketchain-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/pocketchain-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/pocketchain-osx-build.assert
```
...
[pocketchain-project/gitian.sigs.ltc](https://github.com/pocketchain-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
